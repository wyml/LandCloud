...
import { a as n, b as u } from "./chunk-3MNKUQ6L.js";
import { c as b, d as A, f as s } from "./chunk-J4SK6SKL.js";
var N = b((S, p) => {
  var a = function (t) {
  ...
  a.prototype.init_seed = function (t) {
    for (this.mt[0] = t >>> 0, this.mti = 1; this.mti < this.N; this.mti++) {
      ...
      ((this.mt[this.mti] =
        ((((t & 4294901760) >>> 16) * 1812433253) << 16) + (t & 65535) * 1812433253 + this.mti),
        (this.mt[this.mti] >>>= 0));
    }
  };
  a.prototype.init_by_array = function (t, i) {
    var e, o, h;
    for (this.init_seed(19650218), e = 1, o = 0, h = this.N > i ? this.N : i; h; h--) {
      var f = this.mt[e - 1] ^ (this.mt[e - 1] >>> 30);
      ((this.mt[e] =
        (this.mt[e] ^ (((((f & 4294901760) >>> 16) * 1664525) << 16) + (f & 65535) * 1664525)) +
        t[o] +
        o),
        (this.mt[e] >>>= 0),
        e++,
    ...
    for (h = this.N - 1; h; h--) {
      ...
      ((this.mt[e] =
        (this.mt[e] ^
          (((((f & 4294901760) >>> 16) * 1566083941) << 16) + (f & 65535) * 1566083941)) -
        e),
        (this.mt[e] >>>= 0),
        e++,
        e >= this.N && ((this.mt[0] = this.mt[this.N - 1]), (e = 1)));
  ...
  a.prototype.random_int = function () {
    ...
    if (this.mti >= this.N) {
      var e;
      for (this.mti == this.N + 1 && this.init_seed(5489), e = 0; e < this.N - this.M; e++)
        ((t = (this.mt[e] & this.UPPER_MASK) | (this.mt[e + 1] & this.LOWER_MASK)),
...
r.smoothDamp = function (t, i, e, o = 0, h = Number.POSITIVE_INFINITY, f = 1e-4, w = {}) {
  (u.typeOf.number("p", t),
    u.typeOf.number("q", i),
    u.typeOf.number("velocity", e),
    u.typeOf.number.greaterThanOrEquals("deltaTime", o, 0),
    u.typeOf.number.greaterThanOrEquals("maximumSpeed", h, 0),
    u.typeOf.number.greaterThanOrEquals("smoothTime", f, 1e-4),
    u.typeOf.object("result", w),
    (f = Math.max(1e-4, f)));
  let P = 2 / f,
    c = P * o,
    I = 1 / (1 + c + 0.48 * c * c + 0.235 * c * c * c),
    E = h * f,
    m = t - i;
  m = r.clamp(m, -E, E);
  let R = t - m,
    _ = (e + P * m) * o;
  return ((e = (e - P * _) * I), (w.value = R + (m + _) * I), (w.velocity = e), w);
};
r.PI = Math.PI;
...
r.mod = function (t, i) {
  if (!s(t)) throw new n("m is required.");
  if (!s(i)) throw new n("n is required.");
  if (i === 0) throw new n("divisor cannot be 0.");
  return r.sign(t) === r.sign(i) && Math.abs(t) < Math.abs(i) ? t : ((t % i) + i) % i;
};
r.equalsEpsilon = function (t, i, e, o) {
  if (!s(t)) throw new n("left is required.");
  if (!s(i)) throw new n("right is required.");
  ((e = e ?? 0), (o = o ?? e));
  let h = Math.abs(t - i);
  return h <= o || h <= e * Math.max(Math.abs(t), Math.abs(i));
};
r.lessThan = function (t, i, e) {
  if (!s(t)) throw new n("first is required.");
  if (!s(i)) throw new n("second is required.");
  if (!s(e)) throw new n("absoluteEpsilon is required.");
  return t - i < -e;
};
...
r.incrementWrap = function (t, i, e) {
  if (((e = e ?? 0), !s(t))) throw new n("n is required.");
...
r.clamp = function (t, i, e) {
  return (
    u.typeOf.number("value", t),
    u.typeOf.number("min", i),
    u.typeOf.number("max", e),
    t < i ? i : t > e ? e : t
  );
};
var M = new d.default();
r.setRandomNumberSeed = function (t) {
  if (!s(t)) throw new n("seed is required.");
  M = new d.default(t);
};
...
r.acosClamped = function (t) {
  if (!s(t)) throw new n("value is required.");
...
r.chordLength = function (t, i) {
  if (!s(t)) throw new n("angle is required.");
  if (!s(i)) throw new n("radius is required.");
  return 2 * i * Math.sin(t * 0.5);
};
r.logBase = function (t, i) {
  if (!s(t)) throw new n("number is required.");
  if (!s(i)) throw new n("base is required.");
...
r.fastApproximateAtan = function (t) {
  return (u.typeOf.number("x", t), t * (-0.1784 * Math.abs(t) - 0.0663 * t * t + 1.0301));
};
r.fastApproximateAtan2 = function (t, i) {
  (u.typeOf.number("x", t), u.typeOf.number("y", i));
  let e,
    o = Math.abs(t);
  e = Math.abs(i);
  let h = Math.max(o, e);
  e = Math.min(o, e);
  let f = e / h;
  if (isNaN(f)) throw new n("either x or y must be nonzero");
  return (
    (o = r.fastApproximateAtan(f)),
    (o = Math.abs(i) > Math.abs(t) ? r.PI_OVER_TWO - o : o),
    (o = t < 0 ? r.PI - o : o),
    (o = i < 0 ? -o : o),
    o
  );
